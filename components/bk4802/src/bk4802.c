...
static void bk4802_write(i2c_port_t i2c_num, int reg, int data)
{
    ...
    for (int i = 0; i < BK4802_I2C_RETRY; i++) {
        ...
        if (!err) break;
        ESP_LOGW(TAG, "i2c[%d] write fail %x, count = %d, reg = %d, dat = %x", i2c_num, err, i, reg, data);
    }
    if (err) ESP_LOGW(TAG, "i2c[%d] write retry fail %x", i2c_num, err);
}
...
static int bk4802_read(i2c_port_t i2c_num, int reg)
{
    ...
    if (err) ESP_LOGW(TAG, "i2c[%d] read retry fail %x", i2c_num, err);
